...
export default function MaterialTracker() {
  ...
  const username = "Guest"; // Default guest user
  ...
  return (
    <div className="min-h-screen p-6 bg-gray-50">
      ...
      >
        <input name="name" value={form.name} onChange={handleChange} placeholder="Material Name" className="border p-2 rounded w-full md:w-1/4" required />
        <input name="quantity" value={form.quantity} onChange={handleChange} placeholder="Quantity" className="border p-2 rounded w-full md:w-1/4" required />
        <select name="craftType" value={form.craftType} onChange={handleChange} className="border p-2 rounded w-full md:w-1/4">
          <option value="">Select Craft Type (optional)</option>
          {craftTypes.map(ct => <option key={ct} value={ct}>{ct}</option>)}
        </select>
        <input name="imageUrl" value={form.imageUrl} onChange={handleChange} placeholder="Image URL (optional)" className="border p-2 rounded w-full md:w-1/4" />
        <button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">{editing ? "Update Material" : "Add Material"}</button>
      </form>
            ...
            {groupedMaterials[type].map(m => (
              <div key={m._id} className="bg-white p-4 rounded shadow flex flex-col items-center">
                ...
                <div className="flex gap-2 mt-2">
                  <button onClick={() => { setEditing(m._id); setForm({ name: m.name, quantity: m.quantity, craftType: m.craftType, imageUrl: m.imageUrl }); }} className="px-2 py-1 bg-yellow-400 rounded hover:bg-yellow-500">Edit</button>
                  <button onClick={() => handleDelete(m._id)} className="px-2 py-1 bg-red-500 rounded hover:bg-red-600 text-white">Delete</button>
                </div>
              </div>
            ))}
          </div>
